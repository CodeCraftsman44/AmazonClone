...
def getExcludeFields():
    return ['id', 'created_at', 'updated_at', 'domain_user_id', 'added_by_user_id', 'created_by_user_id']
...
def custom_exception_handler(exc, context):
    ...
    if isinstance(exc,AuthenticationFailed):
        ...
        return renderResponse(data=response_data['errors'], message=response_data['message'], status=401)
    elif isinstance(exc, NotAuthenticated):
        return renderResponse(data='User Not Authenticated', message='User Not Authenticated', status=exc.status_code)
    elif isinstance(exc, PermissionDenied):
        return renderResponse(data='You don\'t have permission to access this resource.', message='Permission Denied', status=exc.status_code)
    return response
