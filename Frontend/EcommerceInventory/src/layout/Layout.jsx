...
import {
  CssBaseline, ThemeProvider, createTheme, AppBar, Toolbar, IconButton, Typography,
  Drawer, List, ListItem, ListItemText, Collapse, Divider, Card, CardContent, Fab,
  Box, /* Hidden, */ InputBase, Avatar, Menu, MenuItem, ListItemIcon, BottomNavigation,
  BottomNavigationAction, Link
} from '@mui/material';
...
// Ensure sidebarList is always an array and prevent undefined.map
      ...
      <List sx={{ '& .MuiListItem-root': { transition: 'background-color 0.3s' } }}>
        {(sidebarItems || []).map((sidebarItem, index) => (
          <React.Fragment key={index}>
            <ListItem key={sidebarItem.id} onClick={() => handleSidebarMenuClick(sidebarItem, index)} sx={{ '&.Mui-selected': { backgroundColor: theme.palette.action.selected }, '&:hover': { backgroundColor: theme.palette.primary.light, borderRadius: '10px' } }} className={(sidebarItem?.active && sidebarItem.submenus.length === 0) ? 'active-sidebar' : ''}>
              <ListItemIcon>
                {getIcon(sidebarItem.module_icon || sidebarItem.module_icon)}
              </ListItemIcon>
              <ListItemText primary={sidebarItem.module_name || sidebarItem.name} />
              ...
              <Collapse in={(sidebarItem?.expanded || sidebarItem?.active)} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {sidebarItem.submenus.map(child => (
                    <ListItem sx={{ pl: 4 }} key={child.module_name} onClick={() => handleSidebarMenuClick(child)} className={child?.active ? 'active-sidebar' : ''}>
                      <ListItemIcon>
                        <AlternateEmailIcon />
                      </ListItemIcon>
                      <ListItemText primary={child.module_name || child.name} />
                    </ListItem>
                  ))}
                </List>
              </Collapse>
