...
import { Button, Container, Stepper, Box, LinearProgress } from '@mui/material';
import { Step, StepLabel, Typography, Divider } from '@mui/material';
...
const DynamicForm = () => {
    ...
    return (
        <Container>
            <Typography variant="h6" gutterBottom>Add {formName.toUpperCase()} Form</Typography>
                ...
                <form onSubmit={methods.handleSubmit(onSubmit)}>
                    {formConfig &&
                        ...
                        <>
                            {steps.map((step, index) => (
                                <Box
                                    key={step.fieldType || step.label || index}
                                    component={"div"} sx={{ display: index === currentStep ? 'block' : 'none' }}>
                                    {step.component && <step.component formConfig={formConfig} fieldType={steps.fieldType} />}
                    ...
                    <Box mt={2} display="flex" justifyContent="space-between" >
                        {currentStep > 0 && (<Button type='button' variant="contained" color="primary" onClick={() => goToStep(currentStep - 1)}> <ArrowBackIosIcon /> Back</Button>)}
